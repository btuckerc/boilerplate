...
For a deliberate settings validation on a subset of the next missing pieces, add `"purpose":"validation_test"` to `reserve`. Register the reviewed 3MF normally. Quantities must be a subset of the current next plate shortages; all idle, clear-bed, inspection, hash, and duplicate-send checks still apply. Accept a usable full-size test piece toward the kit only after physical inspection.
