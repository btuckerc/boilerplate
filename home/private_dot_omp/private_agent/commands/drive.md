...
4. Prefer `vibe_spawn` `cli: good` when those tools exist. They exist only after native `/vibe`; if absent, use `task` with the default worker; do not add `vibe.md`.
5. Answer worker questions (`agent://` messages) yourself. Ask the user only if you cannot.
